...
from src.database import connect_db, get_channels_without_metadata, insert_channel_metadata
...
def fetch_channel_metadata_batch(youtube, batch_ids, quota_tracker, batch_num, total_batches):
    ...
    try:
        ...
        logger.info(f"✅ Batch {batch_num}/{total_batches} - Fetched {len(channels)} channels")
...
def fetch_all_channel_metadata():
    ...
    while True:
        try:
            user_input = input(f"\nNumber of channels to process (0-{len(channel_ids)}): ").strip().lower()
            ...
            if user_input in ['all', '0', '']:
                ...
                if 0 < channels_to_process <= len(channel_ids):
                    ...
                    console.print(f"[red]Please enter a number between 1 and {len(channel_ids)}[/red]")
        except ValueError:
    ...
    if channels_to_process < len(channel_ids):
        ...
        console.print(f"[yellow]Limited to processing {channels_to_process} channels[/yellow]")
    ...
    confirm = input(f"\nProceed with fetching metadata for {len(channel_ids)} channels? (yes/no): ").lower()
    if confirm != 'yes':
    ...
    console.print(f"• Rate limiting: {RATE_LIMIT_DELAY} second(s) delay between batch groups")
    ...
    with Progress(
        ...
        for group_start in range(0, len(batches), MAX_PARALLEL_BATCHES):
            ...
            progress.update(task, description=f"Processing batch group {group_start // MAX_PARALLEL_BATCHES + 1}/{(len(batches) + MAX_PARALLEL_BATCHES - 1) // MAX_PARALLEL_BATCHES}")
            
            # Process this group of batches in parallel
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                # Submit all batches in this group
                future_to_batch = {
                    executor.submit(fetch_channel_metadata_batch, youtube, batch_ids, quota_tracker, batch_num, total_batches): (batch_num, batch_ids)
                ...
                for future in as_completed(future_to_batch):
                    ...
                    try:
                        ...
                        if error:
                            ...
                            logger.info(f"✅ Batch {batch_num}/{total_batches} - Processed {len(raw_channels)} channels")
            ...
            if group_start + MAX_PARALLEL_BATCHES < len(batches):
                logger.info(f"⏳ Rate limiting: waiting {RATE_LIMIT_DELAY} second(s) before next batch group...")
